...
allowed = function(url, parenturl)
  ...
  if string.match(url, "^https?://[iv]%.eroshare%.com") then
    return true
  end
  ...
  if string.match(url, "^https?://[^/]*eroshare%.com") then
    ...
    elseif item_type == "user" then
      for id in string.gmatch(url, "([0-9a-zA-Z%-_]+)") do
        if ids[id] == true then
          return true
        end
      end
...
wget.callbacks.get_urls = function(file, url, is_css, iri)
  ...
  if allowed(url, nil) and not string.match(url, "^https?://[iv]%.eroshare%.com") then
    html = read_file(file)
